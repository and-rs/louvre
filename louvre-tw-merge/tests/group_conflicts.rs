use louvre_tw_merge::merge::merge_classes;
use louvre_tw_merge::tw_merge;
...
fn handles_inset_logical_property_conflicts_correctly() {
    ...
    assert_eq!(
        merge_classes("start-full inset-inline-start-auto"),
        "inset-inline-start-auto"
    );
    assert_eq!(
        merge_classes("inset-inline-start-full start-auto"),
        "start-auto"
    );
    assert_eq!(
        merge_classes("end-full inset-inline-end-auto"),
    ...
    assert_eq!(
        merge_classes("top-full inset-block-start-auto"),
        "inset-block-start-auto"
    );
    assert_eq!(merge_classes("inset-block-start-full top-auto"), "top-auto");
    assert_eq!(
        merge_classes("bottom-full inset-block-end-auto"),
        "inset-block-end-auto"
    );
    assert_eq!(
        merge_classes("inset-block-end-full bottom-auto"),
        "bottom-auto"
    );

    // inset-inline/block conflict with inset-x/y (same collision ID)
    assert_eq!(
        merge_classes("inset-x-full inset-inline-auto"),
        "inset-inline-auto"
    );
    assert_eq!(
        merge_classes("inset-inline-full inset-x-auto"),
        "inset-x-auto"
    );
    assert_eq!(
        merge_classes("inset-y-full inset-block-auto"),
        "inset-block-auto"
    );
    assert_eq!(
        merge_classes("inset-block-full inset-y-auto"),
        "inset-y-auto"
    );
    ...
    assert_eq!(
        merge_classes("inset-1 inset-inline-start-full"),
        "inset-1 inset-inline-start-full"
    );
    assert_eq!(
        merge_classes("inset-1 inset-block-start-full"),
        "inset-1 inset-block-start-full"
    );

    // arbitrary values
    assert_eq!(
        merge_classes("start-[10px] inset-inline-start-[20px]"),
        "inset-inline-start-[20px]"
    );
    assert_eq!(
        merge_classes("top-[10px] inset-block-start-[20px]"),
...
fn touch_classes_do_create_conflicts_correctly() {
    assert_eq!(
        merge_classes("touch-pan-x touch-pan-right"),
        "touch-pan-right"
    );
...
fn line_clamp_classes_do_create_conflicts_correctly() {
    assert_eq!(
        merge_classes("overflow-auto inline line-clamp-1"),
        "line-clamp-1"
    );
    assert_eq!(
...
fn text_color_font_size() {
    assert_eq!(
        merge_classes("text-red-500 text-lg"),
